...
class BaselineCalibration():
    # generate dictionary of calibration images from path
    # save median calibration image
    # save median row
    # save centers
    # generate and save lookup tables (sequence + P1 + PN) per amp
    # input test image -> baseline for image
    # 
    def __init__(self, path_to_cal_image_dir, Nsamp, centers, file_strs_list=None, i_amp=0, fitting_func=None, overscan_col_start=None):
        self.path_to_cal_image_dir = path_to_cal_image_dir
        self.Nsamp = Nsamp
        self.centers = centers
        self.i_amp = i_amp
        self.overscan_col_start = overscan_col_start
        if file_strs_list == None:
            print(f"Did not enter file_strs. Using 'ave' and '{self.Nsamp}'")
            self.file_strs_list = ['ave', str(self.Nsamp)]
        else:
            self.file_strs_list = file_strs_list
        self.cal_image_dict = gen_image_dict(path=self.path_to_cal_image_dir, file_strs_list=self.file_strs_list)
        self.median_cal_image = get_median_image(image_dict=self.cal_image_dict, i_amp=self.i_amp, plot=False)
        self.median_cal_row = np.median(self.median_cal_image, axis=0)
        if fitting_func is None:
            self.fitting_func = exp_func
        else:
            self.fitting_func = fitting_func
        self.lookup_tables = self._gen_lookup_tables(new_xs=np.arange(0, 5000))
    
    def plot_median_cal_image(self, origin='origin', aspect='auto'):
        plt.imshow(self.median_cal_image,
                   origin=origin,
                   aspect=aspect)
        plt.title('Median Calibration Image' + fr'{{{self.Nsamp}}}')
        plt.colorbar()
        plt.show();

    def plot_calibration_images(self, origin='upper', aspect='auto'):
        for key in self.cal_image_dict:
            plt.imshow(self.cal_image_dict[key][self.i_amp].data,
                   origin=origin,
                   aspect=aspect)
    ...
    def _gen_lookup_tables(self, new_xs):
        lookup_tables = dict()
        lookup_tables["PN_dN"] = self._gen_lookup_table(y="PN", x_str="dN", new_xs=new_xs, p0=(1.06905824e+03, 7.01978582e-03, 1.63169984e+04))
        lookup_tables["P1_dN"] = self._gen_lookup_table(y="P1", x_str="dN", new_xs=new_xs, p0=(1e5, 1e-1, 1e5))
        lookup_tables["PN_dnu"] = self._gen_lookup_table(y="PN", x_str="dnu", new_xs=new_xs, p0=(1e5, 1e-1, 1e5))
        lookup_tables["P1_dnu"] = self._gen_lookup_table(y="P1", x_str="dnu", new_xs=new_xs, p0=(1e5, 1e-1, 1e5))
        lookup_tables["PN_sequence"] = self._gen_lookup_table(y="PN", x_str="sequence", new_xs=new_xs, p0=(1e5, 1e-1, 1e5))
        return lookup_tables

    def _gen_lookup_table(self, y, x_str, new_xs, p0=(1e5, 1e-1, 1e5)):
        centers = self.centers[np.argwhere(np.diff(self.centers) > 1)[0][0]:]
        if self.overscan_col_start is not None:
            last_center_index = np.argwhere(centers > self.overscan_col_start)[0][0]
        ...
        if x_str == 'dN':
            ...
            if len(ys) == 0:
                ys = self.median_cal_row[500:700]
    ...
    def plot_lookup_tables(self, plot_fit=True, savefig=False):
    ...
    #         print('case 2')
        ...
        if in_N_sequence_region and PN_sequence is not None:  # use PN_sequence if exists
            sequence_cur += 1
            baseline_val += PN_sequence[sequence_cur]
        elif N_cur == 1 and sequence_cur > -1:
            sequence_cur = -1
    ...
    def get_baseline(self, dynamic_var, NROW, NCOL, NROWCLUSTER, NCOLCLUSTER):
        ...
        for i, row in enumerate(baseline):
            ...
            for j, pixel_val in enumerate(row):
                baseline[i, j], sequence_cur = self.get_baseline_val(i_col_cur=j,
